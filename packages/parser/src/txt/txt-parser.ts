...
const CHAPTER_PATTERNS: ChapterPattern[] = [
  {
    name: "标准中文章节",
    pattern: `(?:^\\s*(?:第[ \\t]*)?(?:[零〇一二三四五六七八九十百千万亿0-9]+\\.){0,5}[零〇一二三四五六七八九十百千万亿0-9]+[ \\t]*(?:章|节|卷|部|篇|集|回|话)[ \\t:：]*(.*))`,
    score: 100,
  },
  {
    name: "特殊章节",
    pattern: `(?:^\\s*(?:楔子|序章|前言|引子|开篇|尾声|后记|番外|终章|大结局|完结|后日谈)[ \\t:：]*(.*))`,
    score: 90,
  },
  {
    name: "Markdown 标题",
  ...
  {
    name: "英文章节",
    pattern: `(?:^\\s*(?:chapter|section|part|book|volume)\\s+[\\dIVXLC]+[ \\t:：]*(.*))`,
    score: 80,
  },
  {
    name: "括号标题",
    pattern: `(?:^\\s*【\\s*(?:第[ \\t]*)?[零〇一二三四五六七八九十百千万亿0-9]+\\s*(?:章|节|卷|部|篇|集|回)?\\s*】[ \\t:：]*(.*))`,
    score: 75,
  },
  {
    name: "中文数字列表",
