...
- **Plugin system**: App plugins in `app/src/plugins/`; the plugin runtime (registry/context/loader/types/metadata/store-factory) lives in `app/src/core/plugin-runtime/` are self-contained feature modules. They receive a `PluginContext` on `setup(ctx)` and register capabilities (parsers, modals, overlays, toolbar actions, content transformers). Plugin manifest is build-time generated. Core-owned reader capabilities live in `app/src/core/` (`reader-settings-store`, `reading-progress`, `last-book`, `document-marker`, `reader-session`); `manager`, `settings`, `reading-progress`, and `last-book` are core plugins and cannot be disabled. **Boundary rules, stability tiers and the core/plugin split are defined in `docs/plugin-contract.md` — read it before adding plugin capabilities or moving code into core.** New plugins: copy `app/src/plugins/_template/`.
...
- **Reflowable presentation split**: `ReflowableHost` is the shared iframe/resource/click shell. Mode-specific DOM/measurement logic lives behind `ReflowablePresentation` (`packages/engine/src/presentation.ts`): `PagedController` owns multi-column measurement and `--current-page`; `ScrollController` owns scroll listeners, sentinels, auto-load, restore and calibration.
- **Reader mode vocabulary**: The canonical type is `ReaderMode = "pagination" | "scroll"` in `app/src/utils/reader-mode.ts`. `"vertical"` is only a legacy alias for `"scroll"` accepted when normalizing persisted settings; new code should use the canonical mode everywhere.
- **IndexedDB storage**: `app/src/storage/` is pure persistence — IndexedDB for books, chapters, raw zips, covers, folders, and plugin data. Import/eviction and lazy chapter-extraction orchestration live in `app/src/services/` (`import-book.ts`, `chapter-content.ts`) and call into storage. `createEntityStore<T>` / `createSingletonStore` in plugins provide reactive persistence. Core data lives in its own object stores; plugins get a namespaced key-value adapter.
