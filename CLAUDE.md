...
- **`packages/reader-engine`** — Iframe-based rendering engine. Manages content pipeline (resource injection, chapter resolution), fixed-layout support (PDF/CBZ/CBR), iframe resource loading, and scroll-progress calculation.
...
- **Pagination**: HTML is split into blocks, measured in a hidden offscreen iframe (`reader-engine`), and pages are computed from block positions. Results LRU-cached (capacity 10).
...
Tests use `vitest` (via `vp test`). Current coverage is thin — 3 test files: `packages/reader-core/src/machine.test.ts` (reducer, well covered), `packages/reader-engine/src/scroll-progress.test.ts`, `packages/app/src/plugins/opds/opds-parser.test.ts`. Tests go next to the source files they test.
